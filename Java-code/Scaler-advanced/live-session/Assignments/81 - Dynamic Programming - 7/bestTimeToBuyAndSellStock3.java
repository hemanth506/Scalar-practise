...
public class bestTimeToBuyAndSellStock3 {
    ...
    // Correct approach but Stack Over Flow
    public int maxProfitRecursion(final List<Integer> A) {
        int N = A.size();
        if(N == 0) {
            return 0;
        }
    ...
    public int helper(int i, int j, int[][][] dp, List<Integer> A, int c) {
        if(i == A.size() || c == 0) {
            return 0;
        }
        ...
        if(dp[i][j][c] == -1) {
            if(j == 0) {
                int notBuy = helper(i+1, 0, dp, A, c);
                int buy = -1*A.get(i) + helper(i+1, 1, dp, A, c);
                dp[i][j][c] = Integer.max(notBuy, buy);
            } else {
                int notSell = helper(i+1, 1, dp, A, c);
                int sell = A.get(i) + helper(i+1, 0, dp, A, c-1);
                dp[i][j][c] = Integer.max(notSell, sell);
            }
        }
