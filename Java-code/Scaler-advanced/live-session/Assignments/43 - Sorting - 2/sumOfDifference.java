...
public class sumOfDifference {
    public int solveError (ArrayList<Integer> A) {
        ...
        for(int i = N - 1, j = 0; i >= 0; i--, j++) {
            long bitV = (1<<i)%modV;
            maxV = (maxV + (bitV%modV * A.get(i)))%modV;
            minV = (minV + (bitV%modV * A.get(j)))%modV;
        }
        sumV = (int)(((maxV%modV) - (minV%modV) + modV)%modV);
